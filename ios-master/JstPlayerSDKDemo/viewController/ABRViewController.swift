...
class ABRPlayerViewController: UIViewController {
    ...
    @IBOutlet weak var LogArea: UILabel!
    ...
    var top:CGFloat!
    // MARK: - 定数
    let remoteFileUrl: [String] = ["https://eqd114dmqf.eq.webcdn.stream.ne.jp/www50/eqd114dmqf/jmc_pub/jmc_pd/00005/3de025097f204317ae9c2b27d82582b6.m3u8",
    ...
    func initObserve(){
        ...
        self.videoView.setObserver(name: JstPlayerSDK.DidChangePresentationSize, completion: playerObserve(notification:))
    }
    ...
    func playerObserve(notification:Notification){
        ...
        case JstPlayerSDK.DidStopVideo:
            text = String(notification.name.rawValue)
        case JstPlayerSDK.DidChangePresentationSize:
            text = String(format:"%@ -> %@",String(notification.name.rawValue),NSCoder.string(for: self.videoView.presentationSize ?? CGSize.zero))
        default:
            text = ""
        }
        print(text)
    ...
    func outputLog(text:String){
        let labelText = self.LogArea.text ?? ""
        let format = DateFormatter()
        format.dateStyle = .none
        format.timeStyle = .medium
        format.locale = Locale(identifier: "ja_JP")
        let date = Date()
        self.LogArea.text = labelText + "\r\n" + format.string(from: date) + " " + text
        print(self.LogArea.text)
    }
